...
class CustomizedBusDataset(Dataset):
    ...
    def __init__(self, num_samples, input_size, max_load, max_demand, seed, all_station, travel_time_G, Loc_x, Loc_y, up_down_index, depot, home_id, work_id):
        ...
        for i in range(num_samples):
            ...
            for j in range(len(trip_time[i][0])):
                ...
                decision_state2[i][0][j] = 5 / self.max_time
                decision_state3[i][0][j] = 10 / self.max_time
                decision_state4[i][0][j] = 15 / self.max_time
                decision_state5[i][0][j] = 20 / self.max_time
        ...
        departure_time[:, 0, 0] = 5 / self.max_time
    ...
    def __getitem__(self, idx):
        return (self.static[idx], self.dynamic1[idx], self.dynamic2[idx], self.dynamic3[idx],
                self.dynamic4[idx], self.dynamic5[idx], self.static[idx, :, 0:1], self.problem_record1[idx],
                self.problem_record2[idx], self.problem_record3[idx])
